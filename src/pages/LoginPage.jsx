...
const LoginPage = ({ loginSubmit }) => {
    ...
    const attemptLogin = async (e) => {
        e.preventDefault();
        try {
            const token = await loginSubmit({username: username, password: password});
        ...
        <section className='min-h-screen bg-parchment-100 flex items-center justify-center px-4'>
            <div className='w-full max-w-md'>
                <div className='text-center mb-8'>
                    ...
                    <p className='text-sm text-parchment-500'>Welcome back, reader</p>
                </div>
                <div className='bg-parchment-50 border border-parchment-200 rounded-xl shadow-sm p-8'>
                    ...
                    <form onSubmit={attemptLogin}>
                        ...
                        <p className='text-center text-xs text-parchment-500 mt-4'>
                            Don't have an account?{' '}
                            <Link to='/new-user' className='text-parchment-700 font-medium hover:text-parchment-900 transition-colors'>
                                Create one
                            </Link>
                        </p>
                    </form>
